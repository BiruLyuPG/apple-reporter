...
class Sales {
    constructor(context) {
        function create(type, keys) {
            return (params, { account } = {}) => context.fetch(
...
class Reporter {
    ...
    fetchRaw(serviceUrl, params, { usePassword, account } = {}) {
        return Promise.try(() => {
            const url = `${this.config.baseUrl}${serviceUrl}`;

            let queryInput = `[p=Reporter.properties, ${params}]`;
    ...
    fetch(serviceUrl, params, { usePassword, account } = {}) {
        return this.fetchRaw(serviceUrl, params, { usePassword, account }).then((response) => Reporter.handleFetchResponse(this.config.mode, response));
    }
