...
var Sales = function Sales(context) {
    ...
    function create(type, keys) {
        return function (params) {
            ...
            return context.fetch(context.config.salesUrl, _.compact(_.concat(`Sales.${type}`, util.getParams(keys, params).join(','))).join(', '), { account });
        };
    }
...
var Finance = function Finance(context) {
    ...
    function create(type, keys) {
        return function (params) {
            ...
            return context.fetch(context.config.financeUrl, _.compact(_.concat(`Finance.${type}`, util.getParams(keys, params).join(','))).join(', '), { account });
        };
    }
    ...
    _createClass(Reporter, [{
        ...
        value: function fetchRaw(serviceUrl, params) {
            ...
            var _ref3 = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {},
                usePassword = _ref3.usePassword,
                account = _ref3.account;

            return Promise.try(function () {
                var url = `${_this.config.baseUrl}${serviceUrl}`;

                var queryInput = `[p=Reporter.properties, ${params}]`;

                if (account) {
                    queryInput = `[p=Reporter.properties, a=${account}, ${params}]`;
                }
        ...
        value: function fetch(serviceUrl, params) {
            ...
            var _ref4 = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {},
                usePassword = _ref4.usePassword,
                account = _ref4.account;

            return this.fetchRaw(serviceUrl, params, { usePassword, account }).then(function (response) {
        ...
        value: function retrieveAccessToken() {
            ...
            return this.fetch(this.config.salesUrl, '[Sales.viewToken]', { usePassword: true }).then(function (result) {
                ...
                return _this3.fetchRaw(url, params, { usePassword: true }).then(function (intermediateResponse) {
                    ...
                    return _this3.fetchRaw(`${url}?isExistingToken=Y&requestId=${requestId}`, params, { usePassword: true });
